...
| 12 | 03-gestor-de-encomendas | deviation | tests/e2e/sessao.spec.ts | 110 | 'depois de sair o botao de voltar cai em /login' falhou uma vez (celular) sob a concorrencia da varredura completa (npm run test:e2e sem grep, 8 workers) — timing de navegacao apos logout, nao reproduziu em execucao isolada nem em runs seguintes da mesma varredura. Arquivo da fase 02a, fora do escopo de arquivos do plano 03-08; achado durante a varredura completa que este plano e dono de executar (03-08-PLAN.md full_sweep_responsibility). [CORRIGIDO EM 2026-08-21] O rotulo "contencao de servidor Next unico" desta entrada foi DESMENTIDO POR MEDICAO na sessao de depuracao refresh-nao-chega-no-celular (ver .planning/debug/resolved/): a ida e volta do refresh mede 37-97ms contra orcamento de 10s, e a falha reproduz sem concorrencia. A causa real do #21 e que router.refresh() e um canal COM PERDA (~6% das conclusoes: resposta 200 chega ao navegador, arvore nunca aplicada) — corrigida fechando a tela pela resposta confirmada da Server Action. #12 e #22 continuam open com causa DESCONHECIDA: so o rotulo de contencao caiu. | open |  | 2026-08-09T20:57:11.541Z |  |
