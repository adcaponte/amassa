...
test: "FEITO: varredura completa #1 na 16.3.5 = 3 failed | 33 skipped | 1 did not run | 413 passed (7.2m); família alvo verde; falhas só WINDOWS #3 e #12 (pré-existentes)."
expecting: "Confirmação do dono: o pipeline do CI (depois do push, que é dele) passa nos testes da família e publica."
next_action: "AGUARDANDO verificação humana: o dono faz o push; conferir no CI (gh run watch) que cotacoes-categorias:94, abertura-edicao:165/:234, encomendas-formulario:99 e cotacoes-tracador:44 passam sem retentativa e que o deploy sai. Se confirmar: arquivar em resolved/, fechar WINDOWS #29/#30/#31 e abrir sessão própria para WINDOWS #12 (sessão que volta depois de sair). Se falhar: ler o log com gh run view --log-failed e voltar ao investigating."
...
verification:
  ...
  pendencias_fora_do_escopo:
    - "WINDOWS #12 (sessao.spec.ts:110, 'depois de sair o botão de voltar cai em /login') é um defeito de PRIVACIDADE pré-existente: depois de sair e voltar, o painel protegido aparece. Medido 1/12 na 15.5.22 e 5/12, 7/20 na 16.3.5 (isolado). Duas correções tentadas e revertidas (saída por navegação completa; guarda de bfcache) — nenhuma mudou a taxa, então o servidor enxerga uma sessão VÁLIDA no voltar. Lead medido: toda resposta do app reemite o cookie de sessão (middleware do Auth.js) e há prefetch em voo no instante do 'Sair' — possível ressurreição da sessão por resposta atrasada. NÃO confirmado. Merece sessão própria, com prioridade (aparelho compartilhado). Risco para o CI: com 2 retentativas, ~4% por projeto de barrar o deploy por este teste."
...
commit_local: "faba475 (NÃO enviado — o push é do dono)"
files_changed:
  - package.json
  - package-lock.json
  - tsconfig.json
...
resumo_para_o_dono: |
  ...
  Ficou um ponto que NÃO é desta correção e merece atenção própria: às vezes, depois de "Sair",
  apertar o botão de voltar do celular ainda mostra o painel. Isso já acontecia antes (raramente)
  e, com a versão nova, apareceu mais vezes nos testes. Não corrigi hoje porque as duas tentativas
  rápidas não resolveram e a causa ainda não está provada — está anotado com as medições para a
  próxima sessão. Nada foi enviado ao servidor: o commit é só local; o deploy acontece quando você
  fizer o push.
